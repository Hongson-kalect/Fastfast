...
export const splitSessionIntoDays = (startTimeMs: number, endTimeMs: number): DissectedDay[] => {
  ...
  while (currentPtr < endLimit) {
    ...
    const endOfDay = new Date(year, currentPtr.getMonth(), currentPtr.getDate(), 23, 59, 59, 999);
    ...
    currentPtr = new Date(year, currentPtr.getMonth(), currentPtr.getDate() + 1, 0, 0, 0, 0);
  }
