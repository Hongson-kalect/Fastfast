  ...
  return (
    <View className="flex-row justify-between items-center">
      ...
      <View className="p-1">
        <TouchableOpacity
          activeOpacity={0.7}
          className="h-10 w-10 justify-center items-center rounded-full border-2 border-primary/80 relative"
        >
          {/* Số target hiển thị (thêm z-10 để luôn nổi lên trên dấu chấm mờ nếu cần) */}
          <ThemedText className="text-white/80! text-base!">65</ThemedText>

          {/* 2. Đường TOP (Nằm phía trên border) */}
          <View className="absolute w-0.5 h-2.5 bg-primary/80 z-10 -top-2 left-4.5" />

          {/* 3. Đường BOTTOM (Nằm phía dưới border) */}
          <View className="absolute w-0.5 h-2.5 bg-primary/80 z-10 -bottom-2 left-4.5" />
